...
新規コードでは `Emout.remote()` と `remote_scope()` を起点に書くのを推奨します。
これが worker 側メモリ管理と再利用にいちばん素直な形です:
...
active セッションが保存されていれば、既存コードはそのまま互換モードでも動作します。
互換モードが追従するのは active/default セッションだけです。追加の named session を
使う場合は explicit に接続してください。互換モードの説明は下の
「データ転送モード（互換モード）」にまとめています。

### ローカル field 読み込みを禁止する

ログインノードで大きな field 配列を誤って materialize したくない場合は、
ローカル field データアクセスを無効化できます:
...
この設定は既存の `Emout` インスタンスにも効きます。`Emout(...,
local_data_policy="allow")` を渡したインスタンスだけは明示的にローカル読み込みを
許可できます:
...
`remote_required` 中でも `data.phisp.shape` や `data.phisp.grid_shape` のような
メタデータ参照は可能です。field の実データを読む解析は `data.remote()` /
`RemoteRef` で実行してください。`plot()` は remote session が利用できる場合、
field 配列ではなく画像だけをローカルへ返します。
...
**新規コードでは `Emout.remote()` / `remote_scope()` を推奨**し、こちらは既存コードの
移行負荷を下げるためのモードと考えてください。

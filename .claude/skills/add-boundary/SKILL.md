---
name: add-boundary
description: Add a new MPIEMSES finbound boundary type to emout/core/boundaries/ so data.boundaries[i] handles it. Use when the user asks for support of a new boundary_types(*) entry (e.g. hyperboloid-hole, plane-with-circle-hole, an extension type) or a legacy single-body boundary_type. Covers parameter reading, use_si conversion, _BOUNDARY_CLASS_MAP registration, and the testing pattern.
---
...
Place the new class in `emout/core/boundaries/` near other boundaries of a similar shape (closed solids, planes, legacy pits, …).
...
1. Add the new class name to `__all__` in `emout/core/boundaries/__init__.py`.

...
- `emout/core/boundaries/` — current `_BOUNDARY_CLASS_MAP` and helper functions.
...
5. **Existing Python coverage.** Whether `emout/core/boundaries/` already handles this type, and if so which `Boundary` subclass does it. If not, say so.

...
| 3D plotting raises an import error | Whether `emout.__version__` and PyVista dependencies are current | Update to emout 2.20.0+ and reinstall stale editable environments |
...
- Output of `python -c "import emout; print(emout.__version__)"` and `python -m pip show emout`
- Installation method
